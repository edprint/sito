...
/**
 * Sfondo "liquido" con shader a macchie di colore in un fluido animato;
 * muovendo il mouse le macchie si mescolano (vortice attorno al puntatore).
 * Porting di src/modules/heroScene.ts: il canvas misura il proprio elemento
 * genitore (la .hero__media / .phero__media che lo contiene).
 */
...
export type SixColors = [number, number, number, number, number, number];

const DEFAULT_COLORS: SixColors = [
  0xff5b2e, 0xcdabfe, 0x8fae94, 0xf1e8de, 0xffd21e, 0x2ec5ff,
];
...
export function LiquidCanvas({ colors, className, id }: LiquidCanvasProps): ReactElement {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const colorsRef = useRef<SixColors>(colors ?? DEFAULT_COLORS);
  ...
  useEffect(() => {
    ...
    const renderFrame = (): void => {
      ...
      uniforms.uStrength.value += (strengthTarget - uniforms.uStrength.value) * 0.09;
    ...
    if (reduceMotion) {
      ...
      // pausa il rendering quando il canvas è fuori dallo schermo
