...
/**
 * Animazioni GSAP: intro dell'hero, reveal in scroll, marquee infinito,
 * statement (entrata lettera per lettera + riempimento nero con pin/scrub).
 */
export function initAnimations({ reduceMotion }: { reduceMotion: boolean }): void {
  ...
  // STATEMENT: lettere che entrano, poi si riempiono di nero mentre lo statement
  // è bloccato (CSS sticky). La card che sale è gestita dallo scroll nativo.
  ...
  if (printReveal && stText) {
    ...
    // riempimento nero scrubbato: parte quando lo statement si blocca (top top)
    // e finisce dentro lo spacer, prima che la card inizi a salire
    gsap.to(chars, {
      color: "#111111",
      ease: "none",
      stagger: { amount: 1 },
  ...
  if (footer) {
    ...
    // line-art (illustrazioni + bottiglione): disegno progressivo (stroke-dashoffset)
